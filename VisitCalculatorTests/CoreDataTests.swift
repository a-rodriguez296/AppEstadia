...
class CoreDataTests: XCTestCase {
    ...
    func testAddStay(){
        ...
        let _ = CDStay(dates: [NSDate().endOf(.Day)],taxPayer: taxPayer!,countryCode: colombiaCountryCode, context: NSManagedObjectContext.MR_defaultContext())
        let _ = CDStay(dates: [(NSDate() - 1.days).endOf(.Day)],taxPayer: taxPayer! ,countryCode: colombiaCountryCode , context: NSManagedObjectContext.MR_defaultContext())
        NSManagedObjectContext.MR_defaultContext().MR_saveOnlySelfAndWait()
    ...
    func testWithNonExistingDates() {
        ...
        let datesArray = generateDates0()
        
        let flag = CDDate.verifyDates(datesArray, taxPayer: taxPayer!)
        XCTAssertNil(flag)
    }
    ...
    func testAddingRepeatedDates(){
        ...
        let _ = CDDate(date: NSDate().endOf(.Day),taxPayer: taxPayer!, context: NSManagedObjectContext.MR_defaultContext())
        NSManagedObjectContext.MR_defaultContext().MR_saveOnlySelfAndWait()
        let flag = CDDate.verifyDates([NSDate().endOf(.Day)], taxPayer: taxPayer!)
